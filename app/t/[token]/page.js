...
export default function Customer() {
  ...
  const limit = session
    ? session.guest_equivalent <= 2
      ? 4
      : session.guest_equivalent <= 4
        ? 6
        : session.guest_equivalent <= 6
          ? 8
          : 10
    : 0;
  ...
  return (
    <>
      <div className="topbar">
        <div className="logo">
          MEAT HOUSE<small>UNLIMITED SKEWERS</small>
        </div>
      </div>
          ...
          <>
              ...
              <div className="actions">
                ...
                {session && (
                  <span className="badge new">
                    {session.adults + session.children_8_12 + session.children_4_7 + session.under_4} Guests
                  </span>
                )}
            ...
            {!session ? (
              <div className="notice" style={{ marginTop: 14 }}>
                <b>Your table is not active yet.</b>
                <br />
                Please wait for our team to start your dining session.
              </div>
            ) : (
              <>
                <div className="notice" style={{ marginTop: 14 }}>
                  Choose your skewers below. Your next order becomes available after the table cooldown.
                </div>
                {closed && (
                  <div className="error" style={{ marginTop: 10 }}>
                    Last order has closed for this session.
                  </div>
                )}
                    ...
                    return (
                      ...
                      >
                        <div>
                          ...
                          {wait > 0 && (
                            <div style={{ fontSize: 11, marginTop: 6 }}>Next order in {fmt(wait)}</div>
                          )}
                        </div>
                        <div className="actions" style={{ flexWrap: 'nowrap' }}>
                          <button className="btn secondary small" onClick={() => change(item, -1)}>
                            −
                          </button>
                          <b>{qty}</b>
                          <button
                            className="btn secondary small"
                            disabled={wait > 0 || closed}
                ...
                <div className="card" style={{ position: 'sticky', bottom: 12, marginTop: 16, background: '#241c18', color: '#fff' }}>
                  <div className="actions">
                    <div>
                      <b>{total} skewers</b>
                      <div style={{ fontSize: 12 }}>Limit {total}/{limit} this round</div>
                    </div>
                    <span className="spacer" />
                    <button className="btn gold" disabled={!total || busy || wait > 0 || closed} onClick={submit}>
                ...
                <div className="card">
                  {!data.recent_orders?.length ? (
                    ...
                    data.recent_orders.map((order) => (
                      <div key={order.id} style={{ padding: '9px 0', borderBottom: '1px solid var(--line)' }}>
                        <b>Order {order.round_no}</b> · {order.status}
                        <div style={{ fontSize: 13 }}>
                          {order.order_items.map((item) => `${item.item_name} ×${item.qty}`).join(' · ')}
                        </div>
                      </div>
                    ))
                  )}
