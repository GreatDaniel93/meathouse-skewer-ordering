...
function fmtDate(v) {
  try { return new Intl.DateTimeFormat('en-AU', { day: '2-digit', month: 'short', year: 'numeric', timeZone: 'Australia/Sydney' }).format(new Date(v)); }
  catch { return ''; }
}
...
export default function LuckySkewerReward({ voucher, onClose }) {
  ...
  const validFrom = fmtDate(voucher.redeemable_after);
  const validUntil = fmtDate(voucher.expires_at);
  ...
  return <div className={`lucky-overlay ${ready ? 'show' : ''}`} role="dialog" aria-modal="true" onClick={onClose}>
    ...
    <div className="lucky-stage">
      <div className="lucky-prize" onClick={e => e.stopPropagation()}>
        <div className="lucky-card">
          ...
          <div className="lucky-conditions">
            <b>使用条件 · CONDITIONS</b>
            <span>有效期从明天开始 · Valid from tomorrow{validFrom ? ` (${validFrom})` : ''}</span>
            <span>30天内有效{validUntil ? `，至 ${validUntil}` : ''} · Valid for 30 days</span>
            <span>仅限使用一次 · One-time use only</span>
          </div>
    ...
    <style jsx global>{`
      .lucky-overlay{position:fixed;inset:0;z-index:9999;opacity:0;pointer-events:none}.lucky-overlay.show{opacity:1;pointer-events:auto}.lucky-dim{position:absolute;inset:0;background:rgba(0,0,0,.86);backdrop-filter:blur(4px);animation:luckyDim .32s ease both}.lucky-flash{position:absolute;inset:0;background:#000;animation:luckyFlash .55s ease both}.lucky-fx{position:absolute;inset:0;overflow:hidden;pointer-events:none}.lucky-confetti{position:absolute;top:-25px;width:8px;height:17px;border-radius:2px;opacity:0;animation:luckyFall linear forwards}.lucky-burst{position:absolute;width:12px;height:12px;border:3px solid #ffd166;border-radius:50%;opacity:0;box-shadow:0 0 18px #ff9f1c;animation:luckyRing .9s .32s ease-out forwards}.lucky-stage{position:absolute;inset:0;display:grid;place-items:center;padding:18px}.lucky-prize{width:min(76vw,380px);max-height:88vh;opacity:0;transform:scale(.18) rotate(-7deg);animation:luckyPrize .72s .34s cubic-bezier(.12,1.6,.3,1) forwards}.lucky-card{position:relative;border-radius:22px;padding:16px 15px 15px;color:#28140c;background:linear-gradient(#fffaf0,#ffe4a7) padding-box,linear-gradient(135deg,#fff0a6,#c57900,#fff1aa,#d97706) border-box;border:4px solid transparent;box-shadow:0 30px 70px rgba(0,0,0,.7),0 0 45px rgba(255,174,0,.18);overflow:hidden;font-family:Arial,Helvetica,sans-serif}.lucky-card:after{content:'';position:absolute;inset:-20%;background:linear-gradient(115deg,transparent 42%,rgba(255,255,255,.55) 50%,transparent 58%);transform:translateX(-120%);animation:luckySheen 2.4s 1.4s infinite;pointer-events:none}.lucky-logo{text-align:center;font-weight:900;font-size:16px;letter-spacing:.05em;position:relative;z-index:1}.lucky-logo span{color:#c62828}.lucky-title,.lucky-sub,.lucky-number,.lucky-today,.lucky-won,.lucky-amount,.lucky-voucher-word{font-family:Arial Black,Arial,Helvetica,sans-serif;font-weight:900}.lucky-title{text-align:center;color:#a30f19;font-size:clamp(28px,8.8vw,42px);line-height:.96;margin-top:7px;text-shadow:0 2px #fff,0 4px #e9b441;opacity:0;animation:luckyHit .38s .7s cubic-bezier(.15,1.8,.4,1) forwards}.lucky-sub{text-align:center;font-size:12px;color:#8a3c18;letter-spacing:.06em;margin-top:7px}.lucky-label{text-align:center;margin-top:10px;font-size:12px;font-weight:800}.lucky-number{text-align:center;color:#c5221f;font-size:clamp(56px,18vw,88px);line-height:.78;text-shadow:0 2px #fff,0 5px #f4b942;opacity:0;animation:luckyHit .38s .86s cubic-bezier(.15,1.8,.4,1) forwards}.lucky-number sup{font-size:.27em;vertical-align:top}.lucky-today{text-align:center;font-size:13px;margin-top:6px}.lucky-ticket{margin:14px auto 10px;background:linear-gradient(#cc2323,#901313);border:3px solid #f6c453;border-radius:16px;color:#fff;display:grid;grid-template-columns:auto 1fr;align-items:center;gap:8px;padding:13px 11px;box-shadow:inset 0 0 0 2px #6d0f0f;opacity:0;transform:translateY(12px) scale(.94);animation:luckyUp .38s 1.02s forwards}.lucky-won{font-size:11px;line-height:1.05}.lucky-amount{text-align:center;font-size:clamp(44px,14vw,68px);color:#ffd568;line-height:.8;text-shadow:0 3px #7e4b00}.lucky-voucher-word{text-align:center;font-size:14px;color:#ffd568;margin-top:5px}.lucky-codebox{background:rgba(255,255,255,.62);border:2px dashed #d89c36;border-radius:13px;text-align:center;padding:10px;opacity:0;transform:translateY(8px);animation:luckyUp .38s 1.24s forwards}.lucky-code-label{font-size:10px;font-weight:900;color:#8b4f14;letter-spacing:.07em}.lucky-code{font-size:20px;font-weight:900;color:#a31c15;margin-top:2px}.lucky-notice{font-size:12px;text-align:center;font-weight:900;margin-top:10px;opacity:0;animation:luckyUp .38s 1.38s forwards}.lucky-notice span{display:block;color:#8e3c13;font-weight:800;margin-top:3px}.lucky-conditions{margin-top:9px;padding:8px 10px;border-radius:10px;background:rgba(255,255,255,.48);border:1px solid rgba(158,92,21,.28);font-size:9px;line-height:1.35;text-align:center;color:#64442f;opacity:0;animation:luckyUp .38s 1.5s forwards}.lucky-conditions b{display:block;font-size:10px;color:#8a3c18;margin-bottom:3px}.lucky-conditions span{display:block}.lucky-save{position:relative;z-index:2;width:100%;margin-top:10px;border:0;border-radius:12px;padding:11px 10px;background:#9f1717;color:#fff;font-size:12px;font-weight:900;letter-spacing:.03em;opacity:0;animation:luckyUp .38s 1.68s forwards}.lucky-save:active{transform:scale(.98)}
