...
public class MainActivity extends Activity {
    private EditText ip1,ip2,port,secret;
    ...
    private void buildUi(){
        ...
        TextView brand=text("MEAT HOUSE",32,true,ink); root.addView(brand);
        ...
        TextView hint=text("This phone should stay connected to store Wi-Fi and power.",12,false,Color.DKGRAY); hint.setPadding(0,dp(4),0,0); statusCard.addView(hint); root.addView(statusCard);
        ...
        pcard.addView(label("TCP Port")); port=field("9100",String.valueOf(p.getInt("port",9100))); port.setInputType(InputType.TYPE_CLASS_NUMBER); pcard.addView(port);
        root.addView(pcard);
        ...
        section(root,"BRIDGE ACCESS");
        LinearLayout keyCard=card(); keyCard.setPadding(dp(16),dp(12),dp(16),dp(14));
        keyCard.addView(label("Bridge Key")); secret=field("Bridge Key",p.getString("secret","")); secret.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_VARIATION_PASSWORD); keyCard.addView(secret); root.addView(keyCard);
        ...
        TextView footer=text("ESC/POS · TCP 9100 · Both printers receive the same skewer order",11,false,Color.GRAY); footer.setGravity(Gravity.CENTER); footer.setPadding(0,dp(18),0,0); root.addView(footer);
        setContentView(sv);
    ...
    private void requestBackgroundAccess(){
        ...
        }catch(Exception e){try{startActivity(new Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS,Uri.parse("package:"+getPackageName())));}catch(Exception ignored){}}
    }
    ...
    private void startBridge(){
        try{
            save(); if(secret.getText().toString().trim().isEmpty()){toast("Bridge Key is required");return;}
        ...
        }catch(Exception e){getSharedPreferences(BridgeConfig.PREFS,MODE_PRIVATE).edit().putBoolean("enabled",false).putString("status","Start failed: "+e.getMessage()).apply();toast("Start failed: "+e.getMessage());}
    }
