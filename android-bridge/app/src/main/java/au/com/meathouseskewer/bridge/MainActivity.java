...
public class MainActivity extends Activity {
    ...
    private void buildUi(){
        ...
        TextView hint=text("Green means the server has received a recent heartbeat. 'Bridge running' alone does not mean the server can see this phone.",12,false,Color.DKGRAY);hint.setPadding(0,dp(4),0,0);statusCard.addView(hint);root.addView(statusCard);
    ...
    private final Runnable refresh=new Runnable(){public void run(){SharedPreferences p=getSharedPreferences(BridgeConfig.PREFS,MODE_PRIVATE);String s=p.getString("status",p.getBoolean("enabled",false)?"Starting...":"Stopped");if(statusText!=null)statusText.setText(s);boolean on=p.getBoolean("enabled",false);boolean hb=p.getBoolean("heartbeatHealthy",false);if(statusPill!=null){if(!on){statusPill.setText("● STOPPED");statusPill.setTextColor(Color.rgb(130,130,130));}else if(hb){statusPill.setText("● HEARTBEAT ONLINE");statusPill.setTextColor(Color.rgb(42,120,72));}else{statusPill.setText("● HEARTBEAT OFFLINE");statusPill.setTextColor(Color.rgb(176,55,45));}}if(start!=null)start.setEnabled(!on);if(stop!=null)stop.setEnabled(on);updateBackgroundState();h.postDelayed(this,1000);}};
